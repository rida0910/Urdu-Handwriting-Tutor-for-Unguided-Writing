...
public class CanvasView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public CanvasView(Context context, AttributeSet attrs) {
        ...
        mPaint.setColor(Color.rgb(220, 78, 66));
    ...
    public void draw(float[] arr_x, float[] arr_y)
    {
        clearCanvas();
        startTouch(arr_x[0], arr_y[0]);
        mX = arr_x[0];
        mY = arr_y[0];
        for(int index=1; index < arr_x.length; index++)
        {
            moveTouch(arr_x[index], arr_y[index]);
        }
